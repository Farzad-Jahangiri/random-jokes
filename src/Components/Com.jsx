...
function Com() {
  const [textElementJoke,setTextElementJoke] = useState("Click below to read a joke")

  const BTNgetJokeHandler = ()=>{
    setTextElementJoke("LOADING...")
    GetNewJokes().then((response)=>{
      setTextElementJoke(response.joke)
    });
    // console.log(myJoke);
  }
